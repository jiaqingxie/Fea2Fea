...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        ...
        num_classes = {'Cora': 7, 'PubMed': 3, 'Citeseer': 61}
        #ans = [(0,2)]
        ans = [(0,2,4)]
        aver_test_acc = []
        for value in ans:
            ...
            data =  data.to(device)
            for i in range(10): 
                model =  StrucFeaGNN(concat_fea=True, concat_fea_num = 3,  embed_method = 'GCN', input_dim = data.x.shape[1] + len(value),     output_dim =num_classes[o.dataset], cat_method = 'SimpleConcat', depth = 2, required_batch = False).to(device)
            ...
            print("average test acc: {:.4f}, std: {:.4f}".format(sum(aver_test_acc)/len(aver_test_acc), np.std(aver_test_acc)))
