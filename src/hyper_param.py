            ...
            for thres in threshold:
                ...
                for value in ans:
                    for avg in range(average):
                        ...
                        for epoch in range(1, 200):
                            # for train
                            t_loss = train(value, a.aim_feature, a.dataset, model, 'train', optimizer, train_loader, device, k = 6)
                            # for valid 
                            v_acc = valid(value, a.aim_feature, a.dataset, model, 'valid', optimizer, valid_loader, device,  k = 6)
                            # for test
                            t_acc = test(value, a.aim_feature, a.dataset, model, 'test', optimizer, test_loader, device, k = 6)
                            if v_acc > best_val_acc:
                                best_val_acc = v_acc
                                test_acc = t_acc
                                best_epoch = epoch
                ...
                avg_test_acc = sum(test_acc_arr) / len(test_acc_arr)
                avg_test_acc = round(avg_test_acc, 3)
                log2 = 'threshold : {}, test acc : {:.3f}, std: {:.3f}, task: input {} predict output {}'
                print(log2.format(thres, avg_test_acc, np.std(test_acc_arr, ddof=1), features[a.input_feature], features[a.aim_feature]))
